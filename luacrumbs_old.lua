...
function addFormat(_formatObject)
	table.insert(generateFilesTable, _formatObject)
end
...
function init(_projectname)
	...
	for k,v in pairs(generateFilesTable) do
		v:init(_projectname)
	end
...
function standardInit(verbose)
	...
	if crumbs_generate_hpgl then
		crumbs_hpgl = "IN;PA;SP1;PU0,0;VS5,1;"
	end
end
...
function moveTo(_xpos, _ypos)
	for key,outputFormat in pairs(generateFilesTable) do
		outputFormat:moveTo(_xpos, _ypos)
	end
...
local function moveToZ(_zpos)
	for key,outputFormat in pairs(generateFilesTable) do
		outputFormat:moveToZ(_xpos, _ypos)
	end
	cur_z = _zpos
end
...
-- @params ypos Move to absolute y-position. If nil, the y-value is unchanged
-- @params zpos Move to absolute z-position. If nil, the z-value is unchanged
function oldMoveTo(xpos, ypos, zpos)
...
function oldCircle(radius, counterclockwise)
...
-- If the head is at the current pencilUp position, this command will do nothing (= does not generate any new code)
function pencilUp()
	if (isPencilUp()) then return end
	moveToZ(pencil_up_pos)
	
	--if crumbs_generate_hpgl then
	--	crumbs_hpgl = crumbs_hpgl.."PU"..curx..","..cury..";"
	--end
end

--- Moves the pencil to pencildown-position (z-axis)  
-- Move the head up and down with pencilUp() and pencilDown()
-- If the head is at the current pencilDown position, this command will do nothing (= does not generate any new code)
function pencilDown()
